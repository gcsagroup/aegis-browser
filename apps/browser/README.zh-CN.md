...
- Browser Agent v2 候选源码列出 **95 个顶层 Chromium 补丁**和 **2 个嵌套 V8 补丁**。补丁 0079–0095 已在提交 `6627949a477277dfd916c275267d0bba886f376c` 上通过隔离 Git 索引重放；结果与已提交源码 `c930fa41ef7e9522f145848f3080ee0cc1edc4d8`（tree `33bcb50d8cdd08cfb918af071f3d128b874b15f9`）完全一致。
- 57、65 和 67 补丁记录保留为历史快照，不能给 v2 产物授予资格。
...
- 列入 Chromium 序列的 95 个顶层补丁。
- 2 个应用在嵌套 V8 checkout 中的补丁。
- 57、65 和 67 补丁身份属于历史记录，不覆盖 v2 候选。
- 补丁 0079–0095 已在先前验证的 78 补丁源码树上通过隔离索引精确重放；产物身份和运行资格仍按平台分别判定。
...
95 个顶层补丁和 2 个嵌套 V8 补丁已从各自固定基线通过全新、精确的隔离索引重放。macOS 候选已通过命名的原生/浏览器测试范围，但这仍不满足跨平台身份、受信任签名、公证、已安装分发包或发行授权门禁。
