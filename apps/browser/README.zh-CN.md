...
- Browser Agent v2 候选源码列出 **97 个顶层 Chromium 补丁**和 **2 个嵌套 V8 补丁**。补丁 0079–0095 精确重放到 `c930fa41ef7e9522f145848f3080ee0cc1edc4d8`；补丁 0096 重放到 `f8dff6e3a5dd02527c093b57cde78fc4b0dcb34f`，补丁 0097 生成已提交源码 `a3040bb0dea05e87c2a141b9a29a237a96953620`（tree `d7e560e4803c14a9239c9a9f511f46516f8e1cda`）。
- 57、65、67 和 95 补丁记录保留为历史快照，不能给 97 补丁产物授予资格。
...
- 列入 Chromium 序列的 97 个顶层补丁。
- 2 个应用在嵌套 V8 checkout 中的补丁。
- 57、65、67 和 95 补丁身份属于历史记录，不覆盖当前 v2 候选。
- 补丁 0079–0095 已在先前验证的 78 补丁源码树上通过隔离索引精确重放；补丁 0096 独立生成精确的 96 补丁源码树，补丁 0097 再生成精确的 97 补丁候选树。产物身份和运行资格仍按平台分别判定。
...
97 个顶层补丁和 2 个嵌套 V8 补丁已通过从各自固定基线开始的连续精确隔离索引重放。macOS 候选已通过命名的原生/浏览器测试范围，但这仍不满足受信任签名、公证、已安装分发包或发行授权门禁。
