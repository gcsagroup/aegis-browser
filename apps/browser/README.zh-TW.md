...
106 個頂層補丁和 2 個巢狀 V8 補丁已通過從各自固定基線開始的連續精確隔離索引重放。此前 macOS 原生/瀏覽器測試通過記錄屬於歷史候選，最新語言快取修復須重新建置和回歸。受信任簽署、公證、已安裝散佈套件及發行授權門檻仍未滿足。
