...
The status “in series” means only that the patch file is listed in the current local `series`; it does not mean that the patch has landed upstream, passed release gates, or is publishable. The 49-, 67-, and 95-patch records are retained only as historical snapshots. The current integrated source contains **120 Chromium patches plus 2 nested V8 patches**: 0057–0065 contain the original Browser Agent integration, 0066–0067 contain Settings/update and visual branding, and 0068–0108 replace and harden the v1 execution path with the Browser Agent v2 runtime, cross-platform entry points, scheduled automation, named-site routing, profile isolation, bounded recovery, browser-bound tab/document capabilities, correct product identity, off-UI-sequence persistence, deterministic teardown, and assertion-safe rate-limit handling. Patches 0109–0113 add and harden GitHub browser updates and product status/copy; patches 0114–0116 add access-route planning, trusted policy matching, and atomic Profile rule-store recovery contracts; patch 0117 adds the first fail-closed `net::ProxyInfo` route adapter for registered numeric-loopback HTTP proxy endpoints without a DIRECT fallback; patch 0118 binds exact-host selections to Profile/StoragePartition NetworkContexts through Chromium's native custom-proxy delegate while preserving the native proxy result for non-selected hosts; patch 0119 adds real NetworkService/URLLoader localhost HTTP/HTTPS proxy acceptance cases, including a dead-proxy no-DIRECT regression; patch 0120 expands Chromium GoogleTest regression coverage for proxy identity tuples, partition/selection boundaries, non-selected native routing, and HTTPS dead-proxy fail-closed behavior. Patches 0079–0095 replay to Chromium source commit `c930fa41ef7e9522f145848f3080ee0cc1edc4d8`; patches 0096–0101 produce `1c63ce994b2815fe1f3dc07608ff121d987e0441` (tree `451b3148d12fc2cff2df293cb0f1bb0d6242a908`); patch 0102 produces committed source `13807aaf086948bdff0370e356718d0c2ac54d27` (tree `4546f1afcabf38013ba9bef7e9e5d078ffd3ca77`). Artifact qualification remains platform-specific and requires acceptance evidence for that exact source.
...
| 0117 | Apply fail-closed registered loopback HTTP proxy routes to Chromium `ProxyInfo` without DIRECT fallback | in series |
| 0118 | Bind exact-host localhost HTTP proxy selections to Profile/StoragePartition NetworkContexts | in series |
| 0119 | Add real NetworkService/URLLoader localhost proxy acceptance coverage and dead-proxy fail-closed regression | in series |
| 0120 | Expand Access C++ GoogleTest coverage for ownership, partition, selection, and fail-closed regressions | in series |
