...
- 版本：Plan v2.0-draft
- 日期：2026-08-29
- 状态：**仅架构与原型评估计划，等待确认后才能开发**
- 当前基线：根仓库 `main@cb35227`，Chromium `151.0.7922.77`，67 个顶层补丁 +
  2 个 V8 补丁
- 产品范围：Aegis Chromium Browser 桌面端；iOS 按当前要求跳过，Android 后置
- 授权边界：本文件不授权安装第三方框架、修改源码、调用付费模型、上传浏览器 Profile、
  运行真实交易、推送、部署、签名、公证或发布
...
1. 是否接受“停止继续扩展 v1 执行层，保留安全和原生工具资产，重做 v2 Runtime”。
2. 是否接受先在独立 Profile/工作区比较 P0–P4，不连接用户日常浏览器 Profile。
3. 是否接受 Browser Use/Stagehand 只作原型，正式产品不交付外部通用 CDP 控制面。
4. 原型模型选择：仅本地/loopback，或允许使用用户提供的开发云模型密钥；任何密钥都不
   写入仓库、日志或构建产物。
5. 是否接受原型只做公开只读站点和本地 fixture，不进行真实登录、购物、付款、上传或下载
   后运行。

在以上决策确认前，本文件的结论是：**方案可执行，但开发尚未授权。**
