...
文档索引：[English](../../README.md) · [简体中文](../../README.zh-CN.md) · [繁體中文](../../README.zh-TW.md)。本文件为简体中文设计基线；[冻结清单](freeze.json)记录版本及校验值，[交互示例](interaction-example.html)仅演示本地操作，不连接真实服务。示例为宿主内嵌 HTML 片段，GitHub 文件页仅显示源码。

日期：2026-09-10。文档状态：V1.0 的需求、行为和验收合同已冻结；产品实现及运行验收尚未执行。

本文合并此前设计、最新三策略/两动作与完整性复评，作为 V1.0 开发和验收的唯一行为基线。此前快照与评估稿均为过程草稿，不作为已发布版本。本文中的“必须”是验收条件；标为初始预算的数值也是本版本的测试目标，并非已有性能。真实 VPS 参数、内核版本与构建环境按第 13 节绑定，不能用示例替代。

冻结意味着实现者无需再猜测 ALLOW/BLOCK、冲突、撤销和故障的产品含义；不意味着实现已经证明可行、性能已达标或获准发布。P0 如发现无法满足合同，必须登记失败证据和明确的修订差异，递增规范修订号后重审；不得静默缩小范围、改变语义或放宽指标。具体地址、秘密、负责人和实测容量属于部署绑定项，不是未决的用户行为。当前产品方案版本为 V1.0，冻结修订号为 1。

**1. 目标、范围与完成含义**

普通用户路径固定为：默认直连 → 当前网页点击“一键允许访问” → 必要时自动准备连接 → 应用本站规则并显示结果。节点导入、验证、选择、续期和故障切换均在后台完成，普通用户不操作节点或订阅。
...
| 一键动作 | 对当前页已观察且可处理的拦截/网络失败目标，批量建立精确防护例外与代理路由 |
| 规则策略 | DIRECT / PROXY / REJECT；按明确的网站与目标范围生效，不是全浏览器三挡总开关 |
| 主要操作 | ALLOW：允许所指目标并使用代理；BLOCK：阻止明确选中的目标。批量 ALLOW 保留手动阻断，单目标明确 ALLOW 可解除 |
...
| 开发模式 | 仅 DEV 支持订阅导入/更新、节点参数与手动选择；普通版后台托管 |
| 渠道 | dev / alpha / beta / release，同一套核心实现、不同能力与数据命名空间 |

“一键”表示一次用户操作，不承诺瞬间完成或修复所有网站问题。“全部”限定为当前观察集合及有界补充发现中的合格候选；未知归属、尚未触发的请求和不支持的协议必须明确反映为覆盖不足，不能静默算成功。

本期不建设系统 VPN/TUN，不修改系统代理、DNS 或路由表，不接管其他应用；DIRECT 只表示未使用 Aegis 应用代理，系统既有 VPN/TUN 仍可影响实际网络路径。HTTPS 保持目标站点端到端 TLS，不安装中间人证书。证书、钓鱼页、CSP/CORS 等限制不被本功能关闭。Trojan、Hysteria 2、多区域运营和付费购买流程不属于本期必需交付；未来增加时独立验收。
...
| 仓库提交 | cb35227fcc66f4a451fc93d4b755f3beb0f3a13b；复评时工作区干净 |
| Chromium | 151.0.7922.77，ff37cfca210138f2a40b843b4a8195ab7e4fc7ff |
| 补丁模型 | 67 个 Chromium 补丁、2 个嵌套 V8 补丁；overlay、顺序补丁及构建接线共同交付 |
| 本轮证据 | 已读项目代码及准确 Chromium 提交的接口；未编译本功能、运行 Xray 或压测 VPS |

现有原生工具栏气泡、站点边界计算、OSCrypt 用法与 Profile 工厂范例可以复用；下列状态必须迁移后才能支持多 Profile：

- AegisService 仍是进程单例，初始化另一 Profile 会关闭此前所属状态。[服务定义](https://github.com/gcsagroup/aegis-browser/blob/cb35227fcc66f4a451fc93d4b755f3beb0f3a13b/apps/browser/overlay/chrome/browser/aegis/aegis_service.h#L95)
- BlockReporter 的回调集合、CNAME 可变缓存为进程级状态，必须按真实上下文分区。[报告回调](https://github.com/gcsagroup/aegis-browser/blob/cb35227fcc66f4a451fc93d4b755f3beb0f3a13b/apps/browser/overlay/chrome/common/aegis/aegis_block_reporter.cc#L15)、[缓存定义](https://github.com/gcsagroup/aegis-browser/blob/cb35227fcc66f4a451fc93d4b755f3beb0f3a13b/apps/browser/overlay/chrome/common/aegis/cname_uncloak.h#L21)
- PrivacyEventStore 每文档仅 50 条、总计 200 条，且展示域名已归并；新增精确采集器，不能从展示列表反推授权集合。[事件上限](https://github.com/gcsagroup/aegis-browser/blob/cb35227fcc66f4a451fc93d4b755f3beb0f3a13b/apps/browser/overlay/chrome/browser/aegis/privacy_event_store.h#L30)
- AegisNetThrottle 已会取消请求；仅设置代理无法解除现有拦截。当前 initiator 也不能替代浏览器确认的顶层页面。[当前拦截入口](https://github.com/gcsagroup/aegis-browser/blob/cb35227fcc66f4a451fc93d4b755f3beb0f3a13b/apps/browser/overlay/chrome/common/aegis/aegis_net_throttle.cc#L94)
...
| DevSubscriptionService | DEV 本地订阅解析和调试来源，普通包不注册 | 同上 |
...
自动规则键为：渠道命名空间 + Profile + StoragePartition 策略域 + 顶层 SchemefulSite + 精确目标 host + scheme/port。自动规则不扩展子域、不扩大为整个 Profile；手动扩大作用域时必须显式编辑。
...
保留的有效安装凭据可恢复同一访客权益；卸载或凭据丢失后，不承诺无身份依据地恢复旧访客。服务端可以拒绝再次登记或给予受限权益，不引入隐蔽设备指纹来承诺识别同一人。已认证服务账户通过正常登录恢复原账本。没有配置账户提供方的发行环境只展示访客状态，不显示不可用的登录入口；节点操作始终隐藏。
...
RequestPolicyContext
  ...
  effectiveRuleId, policyGeneration, identityGeneration
...
字段为实现合同，具体序列化版本在 P2 固定。服务端计量标识映射到内部主体，不要求收集真实邮箱。DEV 供应方的请求数元数据使用独立模型，不混入普通版字节用量。
...
顶层目标被拒绝后，显示原生 Aegis 阻断页，说明目标、Profile、规则来源与范围，提供“允许此目标并使用代理 · ALLOW”、规则详情及有权撤销的最近操作。入口属于浏览器可信 UI，不加载被拒绝网站即可使用。离线时 BLOCK/规则查看/本地 DIRECT 编辑仍可用；ALLOW 可发起有界准备，失败保留 REJECT 并显示原因，不能为了进入解除界面先访问目标站。
...
初次分配按 principalId+groupId 生成稳定顺序，先容量组再入口；多个实际容量组采用固定版本的加权 rendezvous 排序，单组退化为组内顺序。散列输入、权重、同分规则和序列化固定并有向量。维护、撤销、到期、冷却和准入不足的候选被排除；权重不代替硬容量限制。已健康绑定不因权重、订阅顺序或延迟轻微变化迁移。
...
DEV 可选 dev_subscription 或 managed_test，来源不能暗中互相回退。订阅只解析支持的节点字段，不导入全局路由、DNS、allow/block 或完整上游策略组；未知格式显示不支持。DEV 手动固定失败时保持并报错，返回自动模式才重新进入正常选择状态机。
...
普通管理页沿用指定参考的卡片顺序：本期流量 → 网络状态 → 网站规则/访问动作 → 脱敏诊断。工具栏气泡提供当前站点的一键入口、数量、作用范围、结果与撤销。直接打开管理页而没有 pageToken 时，不猜测活动标签页替用户授权。
...
当前网站：news.example             当前 Profile：工作
按网站规则 · 默认直连
本页待处理：N 个目标 · 手动阻断 M 项保留
[允许本页 N 个目标 · ALLOW]  [选择目标并阻止 · BLOCK]
目标列表：勾选 / host:port / 策略与来源 / ALLOW / BLOCK
选中后：[阻止所选 M 个目标 · BLOCK]
完成后：已生效 / 已存在 / 保留阻断 / 未解决    [撤销本次操作]
~~~
...
DEV 另有订阅卡、自动保持/手动固定、候选状态、来源/版本和脱敏日志；支持简单/高级视图与分区保存/取消。无修改时保存禁用，切视图不丢草稿，只有校验成功才更新运行配置。普通包不包含可启用这些管理能力的接口和入口。

冻结的 UI 合同如下；接口名为实施时应提供的逻辑能力，可调整语言命名，不得省略参数校验和结果语义。

| 接口 | 输入与必须输出 |
|---|---|
| GetPageAccessSummary / GetPageTargets | browser-owned pageToken；返回有界目标集合、scope、规则版本、mode/来源、可处理与手动拒绝计数、coverage |
...
pageToken 和 ruleId 均须由绑定的 BrowserContext 核验；页面不能借自己传入的 Profile、账户、目标范围或版本扩大权限。普通 UI 不接收任意 proxyGroupId；服务端代理组经本 Profile 准备完成后填充。BLOCK/显式 DIRECT 不因断网、欠额或未登录禁用；仅缺少目标、上下文已失效或无权限时禁用并说明。

同页并发显示中，正在允许不能禁用所选目标的 BLOCK。新 BLOCK 立即使对应旧准备显示“已被后续阻断取代”；迟到结果不把按钮改回已允许。操作成功使用“已允许并代理”或“已阻止”，代理故障使用“PROXY · 等待网络/恢复中/额度耗尽”，不以一个红色状态混淆主动拒绝与服务异常。规则详情的三策略选择和 ALLOW/BLOCK 使用同一执行合同。

EnsureManagedProxyReady、GetAssignmentPlan、Prepare/Commit/RecoverEndpointSwitch 是内部操作，携带幂等 operationId 与版本；普通页面不能指定配置 URL/endpointId。DEV 专属 Import/Refresh/RemoveSubscription、Validate/SelectDebugEndpoint、SetDebugSource/SelectionMode 在普通构建不注册。

UI 使用已有英语、简体中文、繁体中文资源和 Aegis 设计系统；适配宽窄窗口、浅深主题和屏幕阅读器。普通状态明确区分：用户主动阻止、保留手动阻断、准备失败、恢复中、等待网络、流量耗尽、退出身份、到期、同步延迟、保存失败、阻止未完全确认、上下文变化及采集不完整；节点参数和秘密不显示给普通用户。

**10. 渠道、环境、身份与发布边界**

| 渠道 | 允许能力 | 默认服务 |
|---|---|---|
| DEV | 订阅、手动节点、自动策略、详细脱敏诊断；可做优化构建 | local/development 测试身份与用量 |
| Alpha | 零节点配置的普通流程、Alpha 标识和内部诊断；无 DEV 管理能力 | staging 内部租户 |
| Beta | 接近正式体验、Beta 标识与反馈 | staging；明确授权时可进入 production 的独立 Beta 租户 |
| Release | 一键访问、用量、规则及必要诊断 | production 正式租户 |
...
渠道、Profile 的数据目录与凭据命名空间隔离，迁移是明确流程。运行时 Pref、URL、启动参数或远程开关不能提升为 DEV；服务端仍以 issuer/audience/realm/scope/权益鉴权，不信任客户端自报渠道。共享服务器也必须分账和分权限，所有租户共同受真实物理容量约束。
...
| 单元 | 交付内容 | 依赖与完成条件 |
|---|---|---|
| P0 | 固定 checkout 接入验证、两 Profile 三策略、BLOCK 在途/缓存检查、HTTP/SOCKS 认证原型、渠道/安装身份、性能基线和服务资源合同 | 输出实际接入函数与运行证据；确认同步回调外等待点、定向取消能力、连接池参数与真实构建成本 |
| P1 | 既有 AegisService、报告链路及 CNAME 可变状态 Profile 化 | P0；两个普通 Profile 同时运行及销毁顺序均不互相影响 |
...
| P3b | DEV 订阅及来源控制、四渠道编译能力和数据隔离 | P2/P3；普通构建无可启用的 DEV 管理接口 |
...
| P6 | 工具栏 ALLOW/BLOCK、目标列表与三策略详情、原生阻断页、DEV 视图、流量卡和错误状态 | P3b/P3c/P4/P5；首次自动允许、离线阻断、针对目标解除及继承/撤销全流程通过 |
...
| G1：受控 Alpha | 普通流程的有限覆盖版本可试用 | 至少 HTTP→REALITY、两个普通 Profile、三策略/两动作、阻断及恢复、一键联合规则、自动配置/保持/切换、真实字节统计、账户及物理限制、故障不直连；限定用户与测试容量，明确列出未覆盖场景 |
| G2：完整功能候选 / Beta | 本文最终功能覆盖已实现 | P1–P7 完成；A01–A89、A91–A104 适用必需场景及 PF01–PF13 通过；四渠道、两入站/两出站、OTR/Guest 与三策略恢复不能用 Alpha 子集替代 |
| G3：可分发候选 / Release | 在已验证平台、规模和拓扑内可分发 | G2 加 A26/A63–A67/A88/A90 的最终包证据、项目发布门禁、全新安装/升级/回滚；实际发布动作按项目授权执行 |

G1 必查 A01–A05、A09–A11、A13–A17、A19–A23、A27–A31、A35–A40、A47–A60、A68–A78、A81–A86、A91–A104 中与 HTTP→REALITY 普通 Profile 链路对应的子场景，并在 Alpha 实际规模测 PF01–PF13。包含未实现协议的混合用例只能记录“指定子场景通过”，未执行子项仍为 NOT_RUN，整行标 coverage=partial，不标 PASS，不能提前计入 G2。

未完成 HTTP/SOCKS 任一必需入口，或仅有 DEV 订阅没有托管服务，均不能称“完整功能完成”。同样，界面每秒变化但未对账服务端字节，不算用量链路完成。发布阶段不存在通过隐藏失败功能就让同一范围验收通过的例外。
...
| 实际部署 | endpoint→deployment→capacityGroup→trafficPool/failureDomain；别名关系、出口亲和能力及维护策略 | 真实 Alpha 前，运维负责人 |
...
| 计量与额度 | accountingVersion 的固定计数点、lease 大小/TTL、持久化边界、失联结算/预留回收、最大未结算与执行超额字节 | P3c/P3d 集成前冻结，再用故障注入证明；账本负责人 |
...
~~~text
run-manifest.json
  specVersion=V1.0, specRevision=1, frozenSpecHash, testRunId, startedAt, finishedAt
...
以下 A01–A104 为 V1.0 完整功能及交付矩阵；保留此前 A01–A90，A91–A102 对应完整性复评 C01–C12，A103/A104 补齐身份和时效恢复合同。测试实现必须把复合场景拆成可定位的子项。本文交付时均未执行产品验收，不能将“必须观察到的结果”读成实际结果。

| 编号 | 场景 | 必须观察到的结果 |
|---|---|---|
| A01 | 无任何规则访问普通站点 | 直连目标，无代理出口请求 |
| A02 | 后台配置已就绪或 DEV 只导入订阅，未添加规则 | 网页仍直连 |
...
| A24 | 托管/DEV 配置中的 REALITY 与 EdgeTunnel WS+TLS | 不混淆协议组合；不静默接受关闭验证参数 |
| A25 | DEV 订阅失效、更新与节点删除 | 仅在有效期内保留上个有效版本；逻辑代理组有确定的失效处理，网站规则不丢失 |
| A26 | 最终补丁与二进制身份 | 根提交、Chromium/V8 提交、series、GN 参数、Xray 版本、产物 hash 一致 |
| A27 | 全新安装及同安装新普通 Profile，首次一键允许 | 按安装访客合同自动登记/派生独立凭据、验证/选择并继续原批次；无节点表单或二次允许确认；新 Profile 不重复领取独立完整额度 |
...
| A33 | 普通构建尝试 DEV URL/Pref/内部接口 | 不能导入订阅或指定调试节点；DEV 数据不自动迁入普通版本 |
| A34 | DEV 订阅新增、刷新、删除、切换来源 | 来源拥有关系正确，失败状态明确；不悄悄切生产服务；无用量字段不显示为 0 或无限 |
...
| A41 | DEV 订阅附带全局分流、DNS、策略组或未知格式 | 仅接收明确支持的节点字段；网页规则不被覆盖，含凭据订阅不自动发送给第三方转换服务 |
| A42 | DEV 同时存在平台请求数与用户流量额度 | 请求数仅在 DEV 供应方诊断显示，独立标注来源与单位；普通卡片只显示账户字节流量，平台进度不冒充用户余额 |
| A43 | 同一订阅生成多个优选入口 | 候选保留共同来源关系，切入口不新增额度；节点数量不作为出口隔离的证明 |
| A44 | 管理页在 DEV/普通、宽/窄窗口、浅/深主题下展示 | 用量和周期置顶、网络状态卡清楚、管理功能分组正确；普通版没有订阅和节点操作 |
| A45 | DEV 分区编辑、保存失败、取消、切换简单/高级视图 | 无改动时保存禁用，草稿不因视图切换丢失；只有成功校验并保存才更新配置 |
...
| A62 | DEV 手动固定、自动模式恢复、普通版尝试调用调试接口 | 手动固定失败不暗中切换；恢复自动后走同一状态机；普通版无法启用调试选择 |
...
完整功能必须同时满足：普通用户全新 Profile 无节点操作即可一键允许；三策略/两动作和全部转换符合冻结合同，BLOCK 可离线执行、按流终止并保留恢复入口；联合例外与路由原子生效且失败不直连；规则、秘密和状态按渠道/Profile 隔离；自动分配后保持健康绑定、确认故障才有界切换；真实服务端字节用量、额度与固定带宽限制均可验证；两入站、两出站、临时 Profile、身份切换、撤销、恢复与复杂请求覆盖达到 G2。
...
V1.0 冻结修订 1 包含 104 项功能/交付验收、13 项性能指标和 4 个阶段门槛。上述行为缺口均已落实为规范与验收，不留给实现者自行选择语义。真实 VPS 容量、内核版本、RSS 上限等未有实测材料的取值仍按第 13 节绑定，相关门槛在绑定和验证前不通过。
